...
public class Labels {
	...
	public static final String GET_POSTS="SELECT DISTINCT FELADO,POSZT.ID,CIMZETT,IDO,TARTALOM,SZULO  FROM POSZT,FELHASZNALO "
			+"WHERE (FELHASZNALO.ID=POSZT.CIMZETT OR FELHASZNALO.ID=POSZT.FELADO) AND FELHASZNALO.ID IN "
			+"(SELECT DISTINCT ISMER.KIVELID AS ISMID FROM ISMER WHERE KIID=? AND STATUSZ=1 UNION SELECT DISTINCT ISMER.KIID AS ISMID FROM ISMER WHERE KIVELID=? AND STATUSZ=1 UNION SELECT ? FROM DUAL)AND ROWNUM BETWEEN ? AND  ?  AND SZULO IS NULL ORDER BY IDO DESC";
	...
	public static final String GET_ALBUM="SELECT ELERESIUT,ID FROM KEPEK WHERE ALBUMNEV=? AND ALBUMIDO=?";
