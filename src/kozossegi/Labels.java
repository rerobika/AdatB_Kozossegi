...
public class Labels {
		...
		public static final String GET_FRIENDS="SELECT DISTINCT FELHASZNALO.ID,FELHASZNALO.NEV,PROFIL.PROFILKEP "
				+ "FROM ISMER,FELHASZNALO,KEPEK,PROFIL "
				+ "WHERE FELHASZNALO.ID IN "
		...
		public static final String GET_BIRTHDAY="SELECT DISTINCT FELHASZNALO.ID,FELHASZNALO.NEV,PROFIL.PROFILKEP"
				+ "FROM ISMER,FELHASZNALO,KEPEK,PROFIL "
				+ "WHERE FELHASZNALO.ID IN "
				+ "(SELECT DISTINCT ISMER.KIVELID AS ISMID FROM ISMER WHERE ISMER.KIID=? UNION SELECT DISTINCT ISMER.KIID AS ISMID FROM ISMER WHERE ISMER.KIVELID=?) AND FELHASZNALO.ID=PROFIL.SZEMELYID AND (PROFIL.SZUL_DATUM,'YYYY-MM-DD')=(SYSDATE,'YYYY-MM-DD')";
